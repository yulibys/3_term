document.addEventListener('DOMContentLoaded', function () {
  ...
  let lockedToSection2 = false
  let canUnlock = false

  for (let i = 1; i <= total; i++) {
    const num = String(i).padStart(2, '0')
    frames.push(`images/space/frame_${num}.png`)
  }
  ...
  function updateTargetProgress() {
    ...
    const passed = Math.min(Math.max(-rect.top, 0), Math.max(maxScroll, 0))

    targetProgress = maxScroll > 0 ? passed / maxScroll : 0

    if (targetProgress >= 0.999 && !lockedToSection2) {
      lockedToSection2 = true
    ...
    if (window.scrollY < section2.offsetTop - 20 && canUnlock) {
      lockedToSection2 = false
      canUnlock = false
    }
  }
  ...
  function animateFrames() {
    ...
    const index = Math.min(total - 1, Math.round(currentProgress * (total - 1)))
